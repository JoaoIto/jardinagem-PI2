...
export default function AgendaServicos() {
  ...
  const handleDrop = async (event: React.DragEvent<HTMLDivElement>, newStatus: string) => {
    ...
    if (updatedServico) {
      ...
      await fetch(`http://localhost:3001/servicos/${servicoId}`, {
        method: "PATCH",
        headers: {
  ...
  const handleCancel = async () => {
    if (servicoIdToCancel) {
      ...
      const updatedServicos = servicos.map((servico) =>
        servico.id === servicoIdToCancel ? { ...servico, status: "cancelado" } : servico
      );
      setServicos(updatedServicos);

      await fetch(`http://localhost:3001/servicos/${servicoIdToCancel}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleConcludeService = async () => {
    if (servicoIdToConclude) {
      ...
      const updatedServicos = servicos.map((servico) =>
        servico.id === servicoIdToConclude ? { ...servico, status: "concluido" } : servico
      );
      setServicos(updatedServicos);

      await fetch(`http://localhost:3001/servicos/${servicoIdToConclude}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <div className={`${isVerticalLayout ? "flex flex-row" : "flex flex-col"} gap-4 w-full lg:w-4/5 px-4 overflow-auto`}>
        {Object.keys(statusTitles).map((status) => (
          <div
            ...
            style={{
              backgroundColor: hoveredStatus === status ? "rgba(0, 0, 0, 0.1)" : "transparent",
            }}
          >
