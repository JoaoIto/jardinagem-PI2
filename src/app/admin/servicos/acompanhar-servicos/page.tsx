...
export default function AcompanharServicos() {
  ...
  const events = useMemo(() => {
    return servicos.map(servico => {
      ...
      const [day, month, year] = servico.data ? servico.data.split('/') : [1, 1, 2024]
      const eventDate = new Date(Number(year), Number(month) - 1, Number(day))

      if (servico.hora) {
        const [hours, minutes] = servico.hora.split(':').map(Number)
  ...
  const confirmCancel = async () => {
    if (selectedServico) {
      const updatedServicos = servicos.map(servico =>
        servico.id === selectedServico.id ? { ...servico, status: 'cancelado' as const } : servico
      )
      setServicos(updatedServicos)
      await updateServicoStatus(selectedServico.id, 'cancelado')
  ...
  const confirmConclude = async () => {
    if (selectedServico) {
      const updatedServicos = servicos.map(servico =>
        servico.id === selectedServico.id ? { ...servico, status: 'concluido' as const } : servico
      )
      setServicos(updatedServicos)
      await updateServicoStatus(selectedServico.id, 'concluido')
  ...
  <div className="flex justify-center w-full mt-8">
    <Card className="w-full max-w-2xl shadow-lg rounded-lg overflow-hidden border border-gray-200">
      <CardContent className="p-6">
        <Typography variant="h5" className="font-bold text-2xl mb-4 text-verde-normal">{selectedServico.cliente}</Typography>
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            <Typography variant="body2" className="text-gray-500 font-semibold">Descrição</Typography>
            <Typography variant="body1" className="text-gray-800">{selectedServico.descricao}</Typography>
          </div>
          <div>
            <Typography variant="body2" className="text-gray-500 font-semibold">Data e Hora</Typography>
            <Typography variant="body1" className="text-gray-800">{selectedServico.data} - {selectedServico.hora}</Typography>
          </div>
          <div>
            <Typography variant="body2" className="text-gray-500 font-semibold">Valor</Typography>
            <Typography variant="body1" className="text-gray-800">{selectedServico.valor}</Typography>
          </div>
          <div>
            <Typography variant="body2" className="text-gray-500 font-semibold">Telefone</Typography>
            <Typography variant="body1" className="text-gray-800">{selectedServico.telefone}</Typography>
          </div>
        </div>
        <div className="mb-4">
          <Typography variant="body2" className="text-gray-500 font-semibold">Status</Typography>
          <span className={`inline-block px-2 py-1 rounded-full text-sm font-semibold ${
            selectedServico.status === 'aprovado' ? 'bg-blue-100 text-blue-800' :
            selectedServico.status === 'concluido' ? 'bg-green-100 text-green-800' :
            selectedServico.status === 'em-atendimento' ? 'bg-yellow-100 text-yellow-800' :
